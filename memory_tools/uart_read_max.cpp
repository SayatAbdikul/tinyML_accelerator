...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Reading " << COUNT << " bytes from address 0x" << std::hex
            << std::setfill('0') << std::setw(4) << START_ADDR << "..."
            << std::endl;
  ...
  for (size_t i = 0; i < data.size(); i++) {
    std::cout << "  [" << std::dec << i << "] 0x" << std::hex
              << std::setfill('0') << std::setw(4) << (START_ADDR + i)
              << " = 0x" << std::setw(2) << (int)data[i] << " (" << std::dec
              << (int)(int8_t)data[i] << ")" << std::endl;
  }

  // Find index of max value
  int max_idx = 0;
  int8_t max_val = (int8_t)data[0];
  for (size_t i = 1; i < data.size(); i++) {
    if ((int8_t)data[i] > max_val) {
      max_val = (int8_t)data[i];
      max_idx = i + 1; // +1 because we are 1-indexed
    }
  }

  std::cout << "\nMax value: 0x" << std::hex << std::setfill('0')
            << std::setw(2) << (int)(uint8_t)max_val << " (" << std::dec
            << (int)max_val << ")"
            << " at index " << max_idx << " (address 0x" << std::hex
            << std::setw(4) << (START_ADDR + max_idx) << ")" << std::endl;
