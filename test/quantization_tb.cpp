...
int main(int argc, char **argv) {
  ...
  std::vector<std::pair<uint32_t, std::vector<std::pair<int32_t, int8_t>>>>
      tests;
  for (int t = 0; t < 1000; ++t) {
    uint32_t max_abs = std::rand() % 1000000; // Random max_abs in [0, 999999]
    std::vector<std::pair<int32_t, int8_t>> vecs;
    for (int i = 0; i < 8; ++i) {
      int32_t input = (std::rand() % (2 * max_abs + 1)) -
                      static_cast<int32_t>(max_abs); // [-max_abs, max_abs]
  ...
  for (auto &test : tests) {
    ...
    int total_cycles = num_vectors + 20; // 20 cycles to drain pipeline
    for (int i = 0; i < total_cycles; i++) {
      ...
      if (dut->data_valid_out) {
        if (!expected.empty()) {
          ...
          std::cout << "Input: " << std::setw(4) << vectors[out_idx].first
                    << " | Expected: " << std::setw(4)
                    << static_cast<int>(exp_val)
                    << " | Actual: " << std::setw(4)
                    << static_cast<int>(act_val);
