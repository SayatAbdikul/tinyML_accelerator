...
void tick(VerilatedVcdC* tfp, Vtop* dut) {
    dut->clk = 0;
    dut->eval();
...
int main(int argc, char** argv) {
    ...
    if (Verilated::gotFinish() == false) {
        ...
        dut->trace(tfp, 99);
        tfp->open("top_integrated.vcd");
    }
    
    std::cout << "=== Integrated Top Module Test ===" << std::endl;
    ...
    dut->rst = 0;
    std::cout << "Released reset, accelerator should auto-start" << std::endl;
