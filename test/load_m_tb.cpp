...
int main(int argc, char** argv) {
    ...
    while (tiles_seen < 10 && !Verilated::gotFinish()) {
        ...
        if (top->tile_out) {
            ...
            // Access data_out as array of 32-bit words
            uint32_t* words = reinterpret_cast<uint32_t*>(&top->data_out);
            
            // Print with MSB first (big-endian)
            for (int i = WORDS - 1; i >= 0; i--) {
                // Print each 32-bit word as big-endian hex
                for (int j = 3; j >= 0; j--) {
                    uint8_t byte_val = (words[i] >> (j*8)) & 0xFF;
